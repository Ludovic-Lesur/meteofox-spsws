/*
 * nvm_address.h
 *
 *  Created on: 11 aug. 2024
 *      Author: Ludo
 */

#ifndef __NVM_ADDRESS_H__
#define __NVM_ADDRESS_H__

#include "sigfox_types.h"

/*!******************************************************************
 * \enum NVM_address_t
 * \brief NVM address mapping.
 *******************************************************************/
typedef enum {
    // Sigfox library.
    NVM_ADDRESS_SIGFOX_EP_ID = 0,
    NVM_ADDRESS_SIGFOX_EP_KEY = (NVM_ADDRESS_SIGFOX_EP_ID + SIGFOX_EP_ID_SIZE_BYTES),
    NVM_ADDRESS_SIGFOX_EP_LIB_DATA = (NVM_ADDRESS_SIGFOX_EP_KEY + SIGFOX_EP_KEY_SIZE_BYTES),
    // Last wake-up timestamp.
    NVM_ADDRESS_LAST_WAKE_UP_YEAR = (NVM_ADDRESS_SIGFOX_EP_LIB_DATA + SIGFOX_NVM_DATA_SIZE_BYTES),
    NVM_ADDRESS_LAST_WAKE_UP_MONTH = (NVM_ADDRESS_LAST_WAKE_UP_YEAR + 2),
    NVM_ADDRESS_LAST_WAKE_UP_DATE,
    NVM_ADDRESS_LAST_WAKE_UP_HOUR,
    NVM_ADDRESS_LAST_WAKE_UP_MINUTES,
    // Last geolocation timestamp and status.
    NVM_ADDRESS_LAST_GEOLOC_YEAR,
    NVM_ADDRESS_LAST_GEOLOC_MONTH = (NVM_ADDRESS_LAST_GEOLOC_YEAR + 2),
    NVM_ADDRESS_LAST_GEOLOC_DATE,
    NVM_ADDRESS_LAST_GEOLOC_STATUS,
    // Last configuration status.
    NVM_ADDRESS_LAST_CONFIGURATION_UPDATED_FLAG,
    // Last configuration timestamp and status.
    NVM_ADDRESS_LAST_CONFIGURATION_YEAR,
    NVM_ADDRESS_LAST_CONFIGURATION_MONTH = (NVM_ADDRESS_LAST_CONFIGURATION_YEAR + 2),
    NVM_ADDRESS_LAST_CONFIGURATION_DATE,
    NVM_ADDRESS_LAST_CONFIGURATION_STATUS,
    // Weather data period.
    NVM_ADDRESS_WEATHER_DATA_PERIOD,
    // Lux calibration.
    NVM_ADDRESS_LUX_CALIBRATION_GAIN_NUMERATOR,
    NVM_ADDRESS_LUX_CALIBRATION_GAIN_DENOMINATOR = (NVM_ADDRESS_LUX_CALIBRATION_GAIN_NUMERATOR + 2),
    // UV index calibration.
    NVM_ADDRESS_UV_INDEX_CALIBRATION_GAIN_NUMERATOR = (NVM_ADDRESS_LUX_CALIBRATION_GAIN_DENOMINATOR + 2),
    NVM_ADDRESS_UV_INDEX_CALIBRATION_GAIN_DENOMINATOR = (NVM_ADDRESS_UV_INDEX_CALIBRATION_GAIN_NUMERATOR + 2),
} NVM_address_t;

#endif /* __NVM_ADDRESS_H__ */
